...
namespace dso
{
	...
	// 像素点选取标准：
	// 1. 在第0层图像的梯度平方和大于pixelTH0 * thFactor,
	// 2. 在第1层图像的梯度平方和大于pixelTH1 * thFactor,
	// 3. 在第2层图像的梯度平方和大于pixelTH2 * thFactor.
	// 且有先后顺序，先判断前面，如果满足就不判断后面的条件。若都满足，则有资格成为"候选点"。
										  ...
										  float *map_out, int pot, float thFactor)
	{
		...
		int n3 = 0, n2 = 0, n4 = 0;
		for (int y4 = 0; y4 < h; y4 += (4 * pot))	// 4倍步长, pot=3
			for (int x4 = 0; x4 < w; x4 += (4 * pot))
			{
				int my3 = std::min((4 * pot), h - y4);	// 当前大块的实际高度my3和宽度mx3，确保不会超出图像边界
				int mx3 = std::min((4 * pot), w - x4);
				int bestIdx4 = -1;	// 最佳像素点索引
				float bestVal4 = 0;	// 最佳像素点梯度值
				Vec2f dir4 = directions[randomPattern[n2] & 0xF];
				for (int y3 = 0; y3 < my3; y3 += (2 * pot))		// 2倍步长
					for (int x3 = 0; x3 < mx3; x3 += (2 * pot))
					{
						int x34 = x3 + x4;	// 图像中绝对位置
						int y34 = y3 + y4;
						...
						for (int y2 = 0; y2 < my2; y2 += pot)		// 1倍步长
							for (int x2 = 0; x2 < mx2; x2 += pot)
							{
								...
								for (int y1 = 0; y1 < my1; y1 += 1)
									for (int x1 = 0; x1 < mx1; x1 += 1)
									{
										assert(x1 + x234 < w);
										assert(y1 + y234 < h);
										int idx = x1 + x234 + w * (y1 + y234);	// 索引
										int xf = x1 + x234;
										int yf = y1 + y234;

										if (xf < 4 || xf >= w - 5 || yf < 4 || yf > h - 4)	// 边界像素点不考虑
											continue;

										float pixelTH0 = thsSmoothed[(xf >> 5) + (yf >> 5) * thsStep];	// >>5相当于除以32,来缩小索引范围
										...
										if (ag0 > pixelTH0 * thFactor)
										{
											...
											float dirNorm = fabsf((float)(ag0d.dot(dir2)));	// 梯度方向范数
										...
										float ag1 = mapmax1[(int)(xf * 0.5f + 0.25f) + (int)(yf * 0.5f + 0.25f) * w1];	// 像素在第1层的梯度平方和
								...
								if (bestIdx2 > 0)
								{
									map_out[bestIdx2] = 1;		// 标记第2层的最佳像素点
									bestVal3 = 1e10;			// 将第3层的最佳值置为一个很大的数，防止重复选择
								n2++; 							// 增加第2层的像素点数量
								}
							}
